...
class PuzzleRoot(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        self.goat.pos_hint = {"x": -0.5, "y": -0.5}
        ...
        for step in solutionstack:
            if step:
                ...
                if str(step) == "Left, goat":
                    self.goLeft(self.goat, self.goat.pos_hint)
                elif str(step) == "Right, goat":
                    self.goRight(self.goat, self.goat.pos_hint)

    def goLeft(self, image, originalpos):
        """implementer det samme her"""

    def goRight(self, image, originalpos):
        Clock.schedule_once(partial(self._goRight, image))
